...
| 双语对照 | 语言按钮组最后一项（如「English / 中文」）：原文与译文**逐段对齐**并排显示，左右或上下排版可切换；配不上的段落留空框提示 |
| 外观配置 `,` | 见下节 |
...
对照视图不依赖任何标记，只按 Markdown 结构对齐：两份文档各切成段（与「对比（渲染）」用的是同一套切分：标题、代码块、表格、列表、普通段落……），再做一次全局序列对齐——同类型的段优先配对（标题只和同级标题配、代码块只和代码块配），同类型之间有多个候选时，看两段共有的「翻译后不变」的东西：行内代码、链接、数字、英文标识符。只要译文和原文的段落结构大体一致，就能一行行对上；某一侧多出来的段落会单独占一行，另一侧显示为虚线空框。
